    ...
    <Section title="Lichess account">
      {settings.lichessUser ? (
        <div className="flex items-center gap-3 text-sm">
          Connected as <span className="font-semibold">{settings.lichessUser}</span>
        ...
        <div className="flex flex-col gap-3 text-sm">
          ...
          <details>
            <summary className="cursor-pointer text-muted">Or paste a personal API token</summary>
...
function ExplorerSettings({ filter }: { filter: ExplorerFilter }) {
  ...
  return (
    <Section title="Opponent statistics">
      ...
      {filter.db === 'lichess' && (
        <>
          <div className="mb-1 text-xs text-muted">Time controls</div>
          ...
          <div className="mb-1 text-xs text-muted">Opponent ratings</div>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <input
          ...
          onChange={async (e) => {
            ...
            e.target.value = ''
            if (!file) return
            if (!confirm('Importing replaces all repertoires and review history on this device. Continue?')) return
            try {
